...
for i in range(0,len(data)):
    ...
    ymax = max(data.Hitrate[i][0]+data.Hitrate[i][1]+data.Hitrate[i][2]+data.Hitrate[i][3]+data.Hitrate[i][4])
    ymax = (ymax * 1.2)
    my_plot.set_ylim([0,ymax])
    nlines = len(data.Hitrate[i])
    ncol = int(math.ceil(nlines/2.))
    my_plot.legend(["Proxy 1", "Proxy 2","Proxy 3", "Proxy 4","Proxy 5"],ncol = 4, facecolor='white',prop={'size':15},frameon=True)
    fig = my_plot.get_figure()
    fig.autofmt_xdate()
    fig.savefig('figures/Top3/'+data.Run[i]+'.png', bbox_inches='tight')
...
for j in range(0,len(data.Hitrate)):
        daylyAvg = []
        daylyAvg.append((sum(data.Hitrate[j][1][1:169])/169)*100)
        
        daylyAvg.append((sum(data.Hitrate[j][2][1:169])/169)*100)
        
        daylyAvg.append((sum(data.Hitrate[j][3][1:169])/169)*100)
        
        daylyAvg.append((sum(data.Hitrate[j][4][1:169])/169)*100)
...
df["EvictionP"] = df["Eviction"].map(str) + df["EvictionParams"].map(str)
df["AdmissionP"] = df["Admission"].map(str) + df["AdmissionParams"].map(str)
...
for n in df.Size.unique():
    ...
    figur.savefig('figures/Top3/Heatmaps/'+title+'.png', bbox_inches='tight')
...
for j in range(0,len(data.Hitrate)):
        daylyAvg = []
        daylyAvg.append((sum(data.Hitrate[j][1][0:168])/169)*100)
        
        daylyAvg.append((sum(data.Hitrate[j][2][0:168])/169)*100)
        
        daylyAvg.append((sum(data.Hitrate[j][3][0:168])/169)*100)
        
        daylyAvg.append((sum(data.Hitrate[j][4][0:168])/169)*100)
...
for j in range(0,len(data.Hitrate)):
        ...
        TotalHits['Hits'].append(sum(daylyAvg)/sum(daylyAvgWrites))
        TotalHits['Admission'].append(data.Admission[j])
        TotalHits['AdmissionParams'].append(data.AdmissionParams[j])
        TotalHits['Eviction'].append(data.Eviction[j])
        TotalHits['EvictionParams'].append(data.EvictionParams[j])
