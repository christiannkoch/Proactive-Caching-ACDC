...
if(os.listdir('SimulationResults/Top2') != []):
    ...
    for i in indexList:
        ...
        for g in range(0, len(data.loc[i].Hitrate[0])):
            ...
            prox1Delay.append((1-data.loc[i].Hitrate[1][g])*48)
            prox2Delay.append((1-data.loc[i].Hitrate[2][g])*48)
            prox3Delay.append((1-data.loc[i].Hitrate[3][g])*48)
        avgDelay = [(x+y+z+q)/4 for x,y,z,q in zip(prox0Delay,prox1Delay,prox2Delay,prox3Delay)]
